...
NodeIfOp::NodeIfOp( Node *leftptr, Node *midptr, Node* rightptr) {
    type = BIN_OP;
    left = leftptr;
    mid = midptr;
    right = rightptr;
...
NodeDecl1::NodeDecl1(std::string id, Node *expr) {
    type = ASSN;
    identifier = id;
    expression = expr;
}

std::string NodeDecl1::to_string() {
